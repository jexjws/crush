...
func bashDescription(attribution *config.Attribution, modelName string, disableBlocking bool) string {
	bannedCommandsStr := strings.Join(bannedCommands, ", ")
	if disableBlocking {
		bannedCommandsStr = "none"
	}
...
func blockFuncs(disableBlocking bool) []shell.BlockFunc {
	if disableBlocking {
		return nil
	}
...
func NewBashTool(permissions permission.Service, workingDir string, attribution *config.Attribution, modelName string, disableBlocking bool) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		BashToolName,
		string(bashDescription(attribution, modelName, disableBlocking)),
		func(ctx context.Context, params BashParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			...
			if params.RunInBackground {
				...
				bgShell, err := bgManager.Start(context.Background(), execWorkingDir, blockFuncs(disableBlocking), params.Command, params.Description)
				if err != nil {
